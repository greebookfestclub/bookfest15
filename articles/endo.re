...
== 経緯
従来からApple課金の内容把握やチェックにはverifyReceipt API@<fn>{endofn01}を使う必要がありました。サーバーサイドでレシート検証を行うシステムの場合、こちらを利用していることでしょう。しかし先日のWWDC23にてverifyReceitp APIはdeprecatedになりました。ではどのように今後のレシート検証を行えば良いのでしょうか？
...
//indepimage[endo_sequence03][左:従来フロー, 右:新フロー]{
従来フロー
App->Server: レシート
...
方法はこちらの動画でも紹介されています@<fn>{endofn03}。動画にあるようにApple公式ライブラリが用意されている言語(Swift, Java, Node.js, Python)ではそれにしたがってください。今回はphpを想定しているので自前で実装していきます。
//footnote[endofn03][https://developer.apple.com/videos/play/wwdc2023/10143/]
署名検証に必要な証明書はあらかじめAppleのサイトからダウンロードしておきます@<fn>{endofn04}。また、今回参考にしたライブラリはこちらです@<fn>{endofn05}。
...
==== 検証
検証についてはドキュメントに記載があります@<fn>{endofn06}。レシート全体をPKCS #7コンテナとして読み取り、証明書チェーンを抽出し、最終的にAppleの証明書と検証を行います。このようなコードが想定されます。
...
==== 情報取得
レシートのデータは、ASN.1 というフォーマットです。まずデータの中身を取り出す時の定義はこちらです@<fn>{endofn07}。デコードにはphpseclib3のASN.1 Parserが良いでしょう@<fn>{endofn08}。payload取得はこのようなイメージになります。
...
//}
ここで取得したpayloadをデコードする時にはこのような定義を使います@<fn>{endofn09}。payloadを取り出した後、各項目を取り出すためのデータ定義はAppleのサイトにあります@<fn>{endofn10}。
...
== まとめ
Storekit 2に移行するか、もしくは今回のように頑張ってverfiyReceiptのdeprecatedに対応しましょう。
